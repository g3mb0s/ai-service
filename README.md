...
- `POST /chat/conversations/{id}/messages/stream` — отправить сообщение и получить
  ответ потоком SSE (`ready`, `delta`, `done`, `error`);
...
- `GET /characters` — доступные AI-персонажи;
- `POST /characters/{id}/conversations` — начать диалог с персонажем;
- `GET /characters/{id}/conversations` — история диалогов с персонажем;
- `GET /character-conversations/{id}` — диалог с сообщениями и оценками;
- `POST /character-conversations/{id}/messages` — отправить английскую реплику
  и получить короткий ответ персонажа вместе с оценкой `0–10`, исправлением и
...
Первый доступный персонаж — `messi`. Это явно обозначенная ролевая AI-версия,
а не настоящий Lionel Messi. Диалог ведётся только на английском. Ответ и
грамматический комментарий ограничены тремя предложениями; идеальная реплика
получает `quality=10` и пустые `correction`/`comment`, а при ошибке исправление
и комментарий обязательны. Расход этих диалогов входит в общий дневной лимит
пользователя.
